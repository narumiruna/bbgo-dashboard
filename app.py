...
def create_db_url() -> URL:
    db_url = os.environ.get("MYSQL_URL", "mysql+pymysql://root@localhost/bbgo")
    db_url = st.text_input("db_url", value=db_url)
    return db_url
...
def create_nav_chart(engine):
    ...
    fig = px.area(
        nav, x="time", y="net_asset_in_usd", color="currency", line_group="currency"
    )
    st.plotly_chart(fig)
    ...
    daily_nav = (
        nav.groupby(["time"]).sum().drop(columns=["currency"]).resample("D").ffill()
    )
...
def create_daily_profit_chart(engine):
    ...
    profit_start = st.date_input(
        "profit start", (datetime.now() - timedelta(days=365)).date()
    )
...
def create_daily_trade_chart(engine):
    ...
    trade_start = st.date_input(
        "trade start", (datetime.now() - timedelta(days=365)).date()
    )
